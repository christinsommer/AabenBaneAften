...
test('real CP-SAT service and D1: permissions, closed registration, history, validation, preview and atomic draft save', {timeout:180000}, async () => {
  ...
  try {
    ...
    for(let id=1;id<=4;id++) {
      await db.prepare("INSERT INTO players(id,member_no,name,first_name,last_name,gender,self_level,pin_hash,christin_ranking,age) VALUES(?,?,?,?,'Test','M','B','test',5,40)").bind(id,String(id),`Player ${id} Test`,`Player ${id}`).run();
    ...
    await build({entryPoints:['app/api/optimizer/route.ts'],outfile,bundle:true,platform:'node',format:'esm',packages:'external',plugins:[{
      name:'optimizer-test-context',setup(builder) {
        ...
        builder.onResolve({filter:/\/calendar$/},()=>({path:'calendar',namespace:'test'}));
    ...
    tamperScore=false;
    duringSolve=()=>db.prepare('UPDATE players SET age=42 WHERE id=1').run();
    assert.equal((await post()).status,409,'edits during solve invalidate proposal');
    duringSolve=null;
    await db.prepare('UPDATE players SET age=NULL WHERE id=1').run();
