...
async function handleRequest(request, env2) {
  ...
  if (!env2.OPTIMIZER_API_KEY || env2.OPTIMIZER_API_KEY.length < 32) return new Response("Service not configured", { status: 503 });
  const digest = /* @__PURE__ */ __name((value) => crypto.subtle.digest("SHA-256", new TextEncoder().encode(value)), "digest");
  const [expected, actual] = await Promise.all([
    digest(`Bearer ${env2.OPTIMIZER_API_KEY}`),
  ...
  return env2.OPTIMIZER.getByName("main").fetch(request);
}
__name(handleRequest, "handleRequest");

// worker.js
var OptimizerContainer = class extends Container {
  static {
    __name(this, "OptimizerContainer");
  }
  defaultPort = 8080;
  sleepAfter = "2m";
  enableInternet = false;
