...
export function optimizerRevisionSql(eventId: number) {
  return sql`json_array(
    ...
    (SELECT json_group_array(json_array(id,member_no,name,christin_ranking,age,gender,suspended_event_id)) FROM (SELECT * FROM players ORDER BY id)),
...
export async function loadOptimizerInput(eventId: number, weights: unknown) {
  ...
  const members = await db.select({id: players.id, memberNo: players.memberNo, name: players.name, cr: players.christinRanking, age: players.age, gender: players.gender, suspendedEventId: players.suspendedEventId}).from(players).orderBy(asc(players.id));
  ...
  const input: OptimizerInput = {
    players: registrations.filter(s => s.requestedHours > 0 && people.get(s.playerId)?.suspendedEventId !== event.id).map(s => {
      ...
      return {id: p.id, memberNo: p.memberNo, cr: p.cr!, age: p.age, gender: p.gender, availability: JSON.parse(s.availability), requestedHours: s.requestedHours, signupOrder: s.signupOrder!, status: s.status as 'active' | 'waitlist'};
    }),
  ...
  const fingerprintBytes = await crypto.subtle.digest('SHA-256', new TextEncoder().encode(JSON.stringify({revision, weights: input.weights})));
