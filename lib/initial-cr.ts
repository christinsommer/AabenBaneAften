...
export const INITIAL_CR: Record<string,number> = {
  A:2,'A-':3,AB:4,B:5,'B+':4,'B-':6,BC:7,'C+':7,C:8,'C-':9,BEGYNDER:9,
};

export function initialCr(value:string):number|null {
  const normalized=value.trim().toUpperCase();
  if(Object.hasOwn(INITIAL_CR,normalized))return INITIAL_CR[normalized];
